...
static void cosseratRodOde(VectorXd& y_s_out, VectorXd& y){
    //Unpack state vector
...
const int N = 40;
const int Nm1 = N-1;
static void ode4_CosseratRodOde(VectorXd& y0, double L){
    double ds = L/Nm1;
    double half_ds = ds/2;
...
static void jacobianFunction(MatrixXd& J, VectorXd& guess, VectorXd&){
    for(int i = 0; i < 6; i++){
        double L = guess(30+i);
        for(int j = 0; j < 5; j++){
...
static VectorXd guess = (VectorXd(36) << VectorXd::Zero(30), pE(2)*VectorXd::Ones(6)).finished();
...
inline Matrix3Xd getLegCenterline(int i){
    ...
    return ode4<cosseratRodOde,N>(y,L).block<3,N>(0,0);
}
